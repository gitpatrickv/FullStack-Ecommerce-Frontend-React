...
const Star3Rating = ({ productId }: Props) => {
  const [page, setPage] = useState(1);
  const pageSize = 2;
  const { data: userRating } = useGetTotalUserRating(productId);
  const { data: get3StarReviewsAndRating, refetch } =
    useGet3StarRatingAndReview({
      productId,
      pageNo: page,
      pageSize,
    });
  ...
  const isLastPage = get3StarReviewsAndRating?.pageResponse.last ?? false;
  const totalElements =
    get3StarReviewsAndRating?.pageResponse.totalElements ?? 0;
      ...
      <GridItem area="content1">
        <Box>
          {userRating?.total3StarUserRating === 0 ? (
            ...
            <>
              <Box mt="10px">
                {get3StarReviewsAndRating?.ratingAndReviewModels.map(
                  (review) => (
