...
const Star2Rating = ({ productId }: Props) => {
  const [page, setPage] = useState(1);
  const pageSize = 2;
  const { data: userRating } = useGetTotalUserRating(productId);
  const { data: get2StarReviewsAndRating, refetch } =
    useGet2StarRatingAndReview({
      productId,
      pageNo: page,
      pageSize,
    });
  ...
  const isLastPage = get2StarReviewsAndRating?.pageResponse.last ?? false;
  const totalElements =
    get2StarReviewsAndRating?.pageResponse.totalElements ?? 0;
      ...
      <GridItem area="content1">
        <Box>
          {userRating?.total2StarUserRating === 0 ? (
            ...
            <>
              <Box mt="10px">
                {get2StarReviewsAndRating?.ratingAndReviewModels.map(
                  (review) => (
