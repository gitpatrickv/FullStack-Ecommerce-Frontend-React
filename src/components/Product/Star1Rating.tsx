...
const Star1Rating = ({ productId }: Props) => {
  const [page, setPage] = useState(1);
  const pageSize = 2;
  const { data: userRating } = useGetTotalUserRating(productId);
  const { data: get1StarReviewsAndRating, refetch } =
    useGet1StarRatingAndReview({
      productId,
      pageNo: page,
      pageSize,
    });
  ...
  const isLastPage = get1StarReviewsAndRating?.pageResponse.last ?? false;
  const totalElements =
    get1StarReviewsAndRating?.pageResponse.totalElements ?? 0;
      ...
      <GridItem area="content1">
        <Box>
          {userRating?.total1StarUserRating === 0 ? (
            ...
            <>
              <Box mt="10px">
                {get1StarReviewsAndRating?.ratingAndReviewModels.map(
                  (review) => (
