...
const useGet3StarRatingAndReview = ({productId, pageNo, pageSize} : PaginationProps) => {
    return useQuery({
        queryKey: ['ratingAndReview', productId,  pageNo, pageSize],
        queryFn: async () => {
            const {data} = await apiClient.get<RatingAndReviewResponse>(`/product/review/get/3/${productId}`,{
                params: {
                    pageNo: pageNo - 1,
                    pageSize: pageSize
                  } 
